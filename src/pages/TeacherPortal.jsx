...
const TeacherPortal = ({ onNavigate, toggleDarkMode }) => {
    ...
    const loadInitialData = async () => {
        ...
        try {
            ...
            if (isAdmin) {
                // Admins see everything for now or we can implement a selector for them later
                // For simplicity in the teacher portal, admin sees all courses
                const allCourses = await courseService.getCourses();
                setCourses(allCourses);
            } else {
                // 1. Get Instructor Profile by Email
                const instructors = await instructorService.getInstructors();
                const profile = instructors.find(i => i.email?.toLowerCase() === user.email?.toLowerCase());

                if (profile) {
                    setInstructorProfile(profile);
                    currentInstructorName = profile.name;

                    // 2. Load Cources for this instructor
                    const allCourses = await courseService.getCourses();
                    const teacherCourses = allCourses.filter(c =>
                        c.instructor && c.instructor.toLowerCase() === currentInstructorName.toLowerCase()
                    );
                    setCourses(teacherCourses);
                } else {
                    showNotification("No s'ha trobat cap perfil de docent vinculat a aquest email.", "warning");
                }
            }
        } catch (error) {
            console.error("Error loading teacher portal data:", error);
            showNotification("Error al carregar les dades del portal.", "error");
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="space-y-8">
            <header className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4 border-b border-slate-200 dark:border-slate-800 pb-6">
                ...
                <div className="flex flex-col items-end">
                    <div className="flex items-center gap-3 bg-white dark:bg-card-dark px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                        <div className="flex flex-col items-end">
                            <span className="text-[10px] font-black uppercase tracking-widest text-blue-500">{role || 'Usuari'}</span>
                            <span className="text-sm font-bold text-slate-900 dark:text-white">{instructorProfile?.name || user?.email}</span>
                        </div>
                        <div className="w-10 h-10 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center text-blue-600 font-black">
                            {(instructorProfile?.name || user?.email || 'U')[0].toUpperCase()}
                        </div>
                    </div>
                </div>
            </header>
            ...
            <div className="space-y-6">
                ...
                {loading ? (
                    ...
                    <div className="text-center py-20 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700">
                        ...
                        <p className="text-lg font-medium text-slate-500">No tens cursos assignats actualment.</p>
                    </div>
                ) : (
                                        ...
                                        <Button
                                            ...
                                            onClick={async () => {
                                                try {
                                                    ...
                                                    const originalText = "Informe Premium";
                                                    // Temporary simple way to show loading on the specific button
                                                    showNotification("Generant informe...", "info");
                                                    ...
                                                    showNotification("Informe generat amb èxit!", "success");
                                                } catch (error) {
                                                    console.error("Error generating report:", error);
                                                    showNotification("Error al generar l'informe: " + error.message, "error");
                                                }
                                            }}
                                        >
