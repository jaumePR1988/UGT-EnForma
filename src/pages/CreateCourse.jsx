...
const CreateCourse = ({ onBack, toggleDarkMode, onNavigate, onSave, isEditMode = false }) => {
        ...
        const loadCourseData = async () => {
            if (isEditMode && courseId) {
                try {
                    ...
                    showNotification("Error carregant les dades del curs.", "error");
                }
            }
        };
        loadCourseData();
    ...
    const handlePublish = async (status = 'Pendent inici') => {
        ...
        try {
            ...
            if (finalCourseData.heroImage && typeof finalCourseData.heroImage === 'string' && finalCourseData.heroImage.startsWith('data:')) {
                try {
                    ...
                    showNotification("No s'ha pogut pujar la imatge. El curs es guardarà sense imatge de portada.", "warning");
                }
            }

            // 3. Upload Materials
            if (finalCourseData.materials && finalCourseData.materials.length > 0) {
                const uploadedMaterials = await Promise.all(finalCourseData.materials.map(async (material) => {
                    if (material.file) {
                        try {
                            const materialPath = `courses/materials/${Date.now()}_${material.name.replace(/\s+/g, '_')}`;
                            const downloadUrl = await storageService.uploadFile(material.file, materialPath);
                            // Return new object without the File reference
                            const { file, ...rest } = material;
                            return { ...rest, url: downloadUrl };
                        } catch (err) {
                            console.error(`Failed to upload ${material.name}:`, err);
                            return material;
                        }
                    }
                    return material;
                }));
                finalCourseData.materials = uploadedMaterials;
            }
            ...
            showNotification("Error al publicar el curs: " + error.message, "error");
        } finally {
            if (setIsSubmitting) setIsSubmitting(false);
        }
    };
    const publicLink = `${getBaseUrl()}/public/enroll/${courseSlug}`;
    ...
    const handleImageUpload = (e) => {
        ...
        if (file) {
            if (file.size > 5 * 1024 * 1024) {
                showNotification("La imatge és massa gran. Màxim 5MB per evitar problemes amb la base de dades.", "warning");
                return;
            }
    ...
    const handleMaterialUpload = (e) => {
        ...
        files.forEach(file => {
            if (file.size > 20 * 1024 * 1024) {
                showNotification(`El fitxer ${file.name} supera el límit de 20MB.`, "warning");
                return;
            }
                    ...
                    <div className="flex-1 max-w-4xl space-y-10">
                        ...
                        <div className="flex justify-between items-center pt-4">
                            ...
                            <div className="flex space-x-4">
                                ...
                                <button
                                    className="px-6 py-3 border border-slate-300 dark:border-slate-600 rounded-lg font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
                                    onClick={async () => {
                                        setIsSubmitting(true);
                                        try {
                                            await handlePublish('Esborrany');
                                            showNotification("Esborrany guardat correctament.", "success");
                                            onBack();
                                        } catch (e) {
                                            console.error(e);
                                        } finally {
                        ...
                        <section className="bg-card-light dark:bg-card-dark rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                            ...
                            <div className="p-8 space-y-6">
                                <div>
                                    ...
                                    <textarea
                                        className="w-full rounded-lg border-slate-200 dark:border-slate-700 dark:bg-slate-800 focus:ring-primary focus:border-primary transition-all p-3 min-h-[120px]"
                                        placeholder="Afegeix instruccions per als alumnes o una breu descripció dels materials..."
                                        name="description"
                                        value={courseData.description}
                                        onChange={handleInputChange}
                                        id="description"
